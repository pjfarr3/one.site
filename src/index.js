...
// Fake site data (you can edit these later or wire to Supabase)
const initialPlots = [
...
function App() {
  ...
  const [workers, setWorkers] = useState(28);               // fake
  ...
  const [alerts, setAlerts] = useState([
    ...
    { id: 'A-2002', type: 'Car Park', detail: 'Car park full — redirect to overflow', time: '08:22' },
  ]);

  // Live weather (no API key needed)
  ...
  const addFakeDelivery = () => {
    ...
    setDeliveries([{ id: `D-10${n}`, time: 'Now', gate: 'South Gate', for: `Plot ${((n % 5) || 5)}` }, ...deliveries]);
  };

  const addFakeAlert = () => {
    const n = alerts.length + 1;
    setAlerts([{ id: `A-20${n}`, type: 'Intrusion', detail: 'Motion detected near storage', time: 'Now' }, ...alerts]);
  };
      ...
      <aside className="sidebar">
        ...
        <section className="panel">
          ...
          <div className="plotBox">
            ...
            <button onClick={() => { updatePlotStatus(selectedPlot.id); setSelectedPlot(s => ({ ...s, status: cycleStatus(s.status) })); }}>
              Cycle Status
            </button>
          </div>
        ...
        <section className="panel">
          ...
          <p>Voiceflow is pending email verification. We’ll embed it here when ready.</p>
        </section>
      </aside>
    </div>
  );
